...
function thisUriComponent(uriPart){
	
	function getValue(field){
		
		var value = context.current.matchedUri.split("/")[field];
...
function findUser(userid){
    ...
    var ID = "-1";
    var resultObject =  openidm.query(CONFIG_managedObjects.user, { "_queryFilter": "/userName eq \""+ userid +  "\""}, ["_id"]);

    if (resultObject.resultCount == 1 ){
        ID = resultObject.result[0]._id
    }

    console.log("[DEBUG] findUser response: " + ID);
    return ID;
}
